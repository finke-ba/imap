angular.module("imap").controller ("regionController", function ($scope, $http) {
  $scope.circles = [];
  $scope.towns = [];
  ...
  function getRegionChecked () {
    return  $http.get('/imap/region/check').then(function(response) {
      ...
      for (var i = 0; i < response.data.length; i++) {
        ...
        towns[town.townId] = {paramStatus: town.paramStatus, paramStatusId: town.paramStatusId, townId: town.townId};
      }
      return towns;
    });
  }
  ...
  $scope.clickOnCanvas = function(event) {
    ...
    for (var i = 1; i < $scope.towns.length; i++) {
      if (clickedX < $scope.circles[i].right && clickedX > $scope.circles[i].left && clickedY > $scope.circles[i].top && clickedY < $scope.circles[i].bottom) {
        alert ('clicked town ' + $scope.towns[i].townName);
      }
    }
  };
